...
{
	/**
	 * Number of seconds to try for a successful API response
	 * @var int
	 */
	...
	/**
	 * An array of parameters be be returned with a callback
	 * @var array
	 */
	private $callback = [];

	/**
	 * An array of parameters to send to page2images
	 * @var array
	 */
	private $params = [];

	/**
	 * page2images constructor
	 * @param array|null $params_array
	 */
	...
	/**
	 * Sets the number of seconds to try for a successful API response
	 * @param int $seconds
	 * @throws Exception
	 */
	public function setApiTimeout($seconds)
	{
		$s = floor($seconds);
		if ($s < 10 || $s > 180) {
	...
	public function setCallback($callback_array)
	{
		...
		if (is_array($callback_array) === false) {
			throw new Exception ('Callback input must be array');
		}
		foreach ($callback_array as $key => $value) {
			if ($key == '') {
				throw new Exception ('Callback keys cannot be empty');
			}
			$this->callback[$key] = (string) $value;
		}
	}
	...
	public function setDeviceId($id)
	{
		...
		if (in_array($id, ['0', '1', '2', '3', '4', '5', '6', '7', '8']) === false) {
	...
	/**
	 * Sets the required return format
	 * @param $format
	 * @throws Exception
	 */
	public function setFormat($format)
	{
		...
		$f = strtolower($format);
		if (!in_array($f, ['jpg','pdf','png']) === false) {
	...
	/**
	 * Sets full page value
	 * @param mixed $bool
	 */
	public function setFullPage($bool = null)
	{
		if ($bool === null) {
			unset($this->params['p2i_fullpage']);
			return;
		}
		$this->params['p2i_fullpage'] = $this->processBoolean($bool);
	}
	 ...
	 * @param mixed $bool
	 */
	public function setJs($bool = null)
	{
		if ($bool === NULL) {
			unset($this->params['p2i_js']);
			return;
		}
		$this->params['p2i_js'] = $this->processBoolean($bool);
	}
	...
	/**
	 * Handles a callback from page2images
	 * @param string|null $token
	 * @return null|array
	 */
	public static function handleCallback($token = null) {
		// check for valid token
		if ($token !== null && $_GET['page2images_callback_token'] !== $token) {
			return null;
		}
		// get result
		$json = $_POST['result'];
		$result = json_decode($json);
		// get any variables passed back
		$passback = [];
		if ($_GET) {
			foreach ($_GET as $key => $value) {
				if (substr($key, 0, 25) == 'page2images_callback_var_') {
					$passback[substr($key, 25)] = $value;
				}
			}
		}
		// OK
		if ($result->status === 'finished') {
			return [
				...
				'remaining' => $result->left_calls,
				'request' => urldecode($result->ori_url),
				'url' => urldecode($result->image_url),
				'passback' => $passback
			];
		}
		// ERR
		if ($result->status === 'error') {
			return [
				'status' => 'ERR',
				'duration' => $result->duration,
				'message' => '#'.$result->errno.': '.$result->msg,
				'request' => urldecode($result->ori_url),
				'passback' => $passback
			];
		}
		...
		$message = ($result->status == '') ?
			'The page2images API returned no status':
			'The page2images API returned unrecognised status ('.$result->status.')';
		$message .=  ($result->msg == '') ?
			' with no message' :
	...
	/**
	 * Checks parameters are as required by page2images
	 * @throws Exception
	 */
	private function checkParams()
	{
		...
		if ($missing) {
			throw new Exception ('Call cannot be made, required parameters (' . implode(', ', $missing) . ')are missing');
		}
		...
		if ($this->params['p2i_quality']) {
			...
			if ($format === 'png' && ($quality < 70 || $quality > 85)) {
				throw new Exception ('Image quality must be between 70 and 85 for png');
			}
			if ($format === 'jpg' && ($quality < 80 || $quality > 95)) {
	...
	private function prepareCallback()
	{
		...
		if ($this->callback['url'] == '' || is_string($this->callback['url']) === false) {
	 ...
	 * @return array
	 */
	private function processApiError($error_id, $error_msg)
	{
		switch (substr($error_id . $error_msg, 0, 13)) {
			case '403The API Ke':
				$id  = '5';
				$msg = 'Invalid API key';
				break;
			...
			default:
				$id  = '11';
				$msg = 'Unexpected error returned by API (' .
					$error_id . ': ' . $error_msg . ')';
		}
		return [
			'id' => $id,
	...
	/**
	 * Cleans submitted boolean values
	 * @param mixed $bool
	 * @return string
	 * @throws Exception
	 */
	private function processBoolean($bool)
	{
		if (in_array($bool, [0, '0', false, 'false'], true)) {
			return '0';
		}
